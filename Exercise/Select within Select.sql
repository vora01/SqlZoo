...
SELECT name FROM world 
WHERE continent LIKE 'Europe' AND GDP/population > (SELECT GDP/population FROM world WHERE name = 'United Kingdom')
...
WHERE population > ALL (SELECT population*3 FROM world y WHERE 
                             y.continent=x.continent AND y.name != x.name)
